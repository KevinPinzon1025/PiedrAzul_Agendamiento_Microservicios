...
public class SelfServiceAppointmentController {
    ...
    public SelfServiceAppointmentController(SelfServiceAppointmentFrame view) {
        this.view = view;
    ...
    public void initData() {
        try {
            ...
            view.cbProfessional.getItems().setAll(professionals);
            // El paciente no elige paciente, el sistema debería detectar su sesión
            // Aquí omitimos cbPatient ya que es "Self-Service"
        } catch (Exception e) {
            view.lblFeedback.setText("Error al cargar médicos disponibles.");
        }
    }

    public void setupHelpListeners() {
        addHelp(view.cbProfessional, "Seleccione al médico especialista con el que desea agendar su cita.");
        addHelp(view.datePicker, "Elija el día de su preferencia. Solo se permiten fechas futuras.");
        addHelp(view.cbTime, "Seleccione un horario disponible para la fecha elegida.");
        addHelp(view.txtMotivo, "Describa brevemente el síntoma o razón de su visita para que el médico esté preparado.");
    }

    private void addHelp(Node node, String message) {
        node.setOnMouseEntered(e -> view.lblHelpBox.setText(message));
        node.setOnMouseExited(e -> view.lblHelpBox.setText("Pase el cursor sobre un campo para ver la ayuda."));
    }
    ...
    public void handleRegister() {
        try {
            if (validateFields()) {
                ...
                // TODO: En un sistema real, el PatientId vendría del usuario logueado
                request.setProfessionalId(httpClient.getProfessionalIdByName(view.cbProfessional.getValue()));
                request.setObservation(view.txtMotivo.getText());
                request.setAppointmentDate(view.datePicker.getValue().atTime(10, 0));
                request.setSchedulingType(SchedulingType.SELFSERVICE);

                httpClient.createAppointment(request);
                view.lblFeedback.setStyle("-fx-text-fill: green;");
                view.lblFeedback.setText("¡Cita agendada con éxito!");
                clearForm();
            }
        } catch (Exception ex) {
            view.lblFeedback.setStyle("-fx-text-fill: red;");
            view.lblFeedback.setText("Error al procesar la solicitud.");
        }
    }

    private boolean validateFields() {
        if (view.cbProfessional.getValue() == null || view.datePicker.getValue() == null || view.txtMotivo.getText().isBlank()) {
            view.lblFeedback.setText("Por favor complete todos los campos.");
            return false;
        }
        return true;
    }
    ...
    public void clearForm() {
        ...
        view.cbTime.getSelectionModel().clearSelection();
